const countries = ([
  {
    id: "11",
    d: "m72,170l-33,-18l-20,-58l36,-38l28,29l56,-17l35,45l-34,51l-68,6z",
    fill: "hsl(99, 43%, 30%)",  //Calculate on data load
    stroke: "#000000",
    name: "Country A",
    center: [100, 120],  // Example center point
    isSupplyPoint: false,  // No supply point
    occupyingArmy: {Id: 11, OwnerId: 1},
    ownerId: 1,
    adjacentCountriesById: [ 12, 13, 15]
  },
  {
    id: "12",
    d: "m90,187l65,-6l36,-60l49,-2l42,65l-82,10l-68,49l-40,-25l-2,-31z",
    fill: "hsl(0, 0%, 66%)",
    stroke: "#000000",
    name: "Country B",
    center: [170, 190],  // Example center point
    isSupplyPoint: true,  // This country has a supply point
    occupyingArmy: null,
    ownerId: null,
    adjacentCountriesById: [ 11, 13, 14, 15]
  },
  {
    id: "13",
    d: "m194,102l-32,-36l70,-40l95,28l-15,47l-47,22l-20,-24l-51,3z",
    fill: "hsl(99, 43%, 30%)",
    stroke: "#000000",
    name: "Country C",
    center: [240, 70],  // Example center point
    isSupplyPoint: false,  // No supply point
    occupyingArmy: {Id: 8, OwnerId: 1},
    ownerId: 1,
    adjacentCountriesById: [ 11, 12, 14 ]
  },
  {
    id: "14",
    d: "m276,139l51,-23l14,-48l47,26l14,67l-38,20l-69,-2l-19,-40z",
    fill: "hsl(278, 70%, 30%)",   
    stroke: "#000000",
    name: "Country D",
    center: [340, 130],  // Example center point
    isSupplyPoint: false,
    occupyingArmy: {Id: 15, OwnerId: 2},
    ownerId: 2,
    adjacentCountriesById: [ 12, 13, 17]
  },
  {
    id: "15",
    d: "m74,185l-37,-16l-21,59l27,77l44,13l45,-57l-60,-34l2,-42z",
    fill: "hsl(0, 0%, 66%)",  
    stroke: "#000000",
    name: "Country E",
    center: [60, 250],  // Example center point
    isSupplyPoint: false,
    occupyingArmy: null,  // No occupying army
    ownerId: null,
    adjacentCountriesById: [ 11, 12, 16 ]
  },
  {
    id: "16",
    d: "m104,326l45,-56l26,35l54,0l24,-19l27,15l-62,44l-114,-19z",
    fill: "hsl(278, 70%, 30%)",   
    stroke: "#000000",
    name: "Country F",
    center: [180, 310],  // Example center point
    isSupplyPoint: true,  // This country has a supply point
    occupyingArmy: {Id: 16, OwnerId: 2},
    ownerId: 2,
    adjacentCountriesById: [ 15, 17, 18 ]
  },
  {
    id: "17",
    d: "m267,272l35,-39l10,-39l58,-1l24,-4l18,70c0,0 -51,6 -52,6c-1,0 -71,25 -71,25c0,0 -22,-18 -22,-18z",
    fill: "hsl(278, 70%, 30%)",   
    stroke: "#000000",
    name: "Country G",
    center: [350, 230],  // Example center point
    isSupplyPoint: false,
    occupyingArmy: null,
    ownerId: 2,
    adjacentCountriesById: [ 14, 16, 18]
  },
  {
    id: "18",
    d: "m256,338l43,-37l62,-22l30,-4l-20,52c0,0 -35,17 -37,19c-2,2 -61,10 -62,10c-1,0 -16,-18 -16,-18z",
    fill: "hsl(278, 70%, 30%)",   
    stroke: "#000000",
    center: [330, 320],  // Center point for Småland
    name: "Småland",
    isSupplyPoint: true,  // Småland is a supply point
    occupyingArmy: {Id: 3, OwnerId: 2},  // Assuming occupyingArmy is represented as "Army 11"
    ownerId: 2,
    adjacentCountriesById: [ 16, 17]
  }
]);

export default countries;